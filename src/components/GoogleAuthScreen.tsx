    ...
    <div className="login-screen">
      <div className="login-card fade-in relative">
        ...
        <div className="space-y-4 relative">
          {showPinPrompt ? (
            <div className="bg-slate-50 border border-slate-200 p-4 rounded-2xl animate-in fade-in zoom-in-95">
              <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest mb-3">Masukkan PIN Guest</p>
              <div className="flex gap-2">
                <input 
                  ...
                  placeholder="PIN..."
                  className="flex-1 bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-black text-center tracking-widest focus:outline-none focus:border-blue-500"
                  maxLength={4}
                />
                <button 
                  onClick={handleGuestLogin}
                  className="bg-blue-600 text-white px-5 rounded-xl font-black text-[10px] tracking-widest uppercase hover:bg-blue-700 transition-colors"
                >
                  Masuk
                </button>
              </div>
              <button 
                onClick={() => { setShowPinPrompt(false); setGuestPin(''); }}
                className="w-full mt-3 text-[10px] font-black text-slate-400 hover:text-slate-600 uppercase tracking-widest"
              >
                Batal
              </button>
            </div>
